...
//        $invoice->number($order->id);
        ...
        $user = \App\Models\Sistema\User::find($order->user_id);
...
if(!function_exists('redirectWithError')){
    /**
     * Redireciona para a url contida em $fullUrl caso $redirect = true
     * 
     * @param boolean $redirect 
     * @param string  $fullUrl - Ex: mysite.com/user/cursos/turmas?proposta_id=1
     * @param string  $withMessage - Texto da mensagem para exibir um alerta (por exemplo)
     *
     *  @return void
     */
    function redirectWithError(bool $redirect, string $fullUrl, string $withMessage)
    {
        $withMessage = !empty($withMessage) ? $withMessage : __('Não foi possível concluir a solicitação.');
        if ($redirect) {
            throw_if(
                $redirect,
...
if(!function_exists('redirectWithSuccess')){
     ...
     *  @return void
     */
    function redirectWithSuccess(bool $redirect, string $fullUrl, string $withMessage)
    {
        if ($redirect) {
            throw_if(
                $redirect,
